(function ($, window, document, undefined) {
    ...
    classes.Scroller = function (el, settings, inherit) {
        ...
        function onStart(ev) {
            // todo
            if($(ev.target).hasClass('other-input')){
                // return;
            }
            // Scroll start
        ...
        function generateWheelItems(i) {
            ...
            $.each(values, function (j, v) {
                ...
                var content = '';
                if (typeof keys[j] === 'string' && keys[j].split(',')[1] === '-1'){
                    //生活のブロックタブ その他
                    content = '<input type="text" class="other-input" maxlength="30" placeholder="' + v + '"/>';
                }else{
                    content = v;
                }
                html += '<div role="option" aria-selected="false" class="dw-li dw-v" data-val="' + keys[j] + '"' + (labels[j] ? ' aria-label="' + labels[j] + '"' : '') + ' style="height:' + itemHeight + 'px;line-height:' + itemHeight + 'px;">' +
                    '<div class="dw-i"' + (lines > 1 ? ' style="line-height:' + Math.round(itemHeight / lines) + 'px;font-size:' + Math.round(itemHeight / lines * 0.8) + 'px;"' : '') + '>' + content + '</div></div>';
                l++;
            });
