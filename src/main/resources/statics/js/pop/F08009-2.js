$(function () {
    ...
    $("#conf").click(function () {
        ...
        $.get(ctxPath + "/manager/F08009/getDspCount",
            function (rsp) {
                if (rsp.dspcount <= 0) {
                    //若dsp表中不存在該用戶的設定
                    $.post(ctxPath + "/manager/F08009/saveDspItems", {
                        dspitems: JSON.stringify(colList)
                    });
                } else {
                    //若dsp表中存在該用戶的設定
                    $.post(ctxPath + "/manager/F08009/updDspItems", {
                        dspitems: JSON.stringify(colList)
                    });
                }
            });
        parent.reload(colList, hiddenList, null, null, null, null, null, null, null);
