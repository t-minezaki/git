    ...
    methods: {
        setUp: function () {
            ...
            $('#dayView').click(function () {
                ...
                window.location.href = "./F21028.html?inner=" + true + "&schyDiv=" + $('#schySelect').val() + "&grpId=" + $('#grpSelect').val() + "&stuNm=" + encodeURI($('#stuNmInput').val());
            });
            //「月」ボタンクリックイベント
            ...
            this.date = new Date();
            //最小週の日曜日
            this.date.setDate(this.date.getDate() - this.date.getDay() - 20);
            //「来週」アイコンのイベントをクリック
            $('#nextWeek').click(function () {
                vm.isFirst = true;
                vm.pageTurning = true;
                vm.date.setDate(vm.date.getDate() + 49);
                vm.getResult();
            });
            //「先週」アイコンのイベントをクリック
        ...
        getResult: function () {
            ...
            for (var i = 0; i < 7; i++) {
                ...
                columns.push({
                    ...
                    cellattr: function (rowId, val, rawObject, title, rdata) {
                        ...
                        if (vm.standEntity.perfStandWeek3 && vm.standEntity.perfStandWeek2 && vm.standEntity.perfStandWeek1){
                            if (rawObject[title.name] === -1){
                                return "style='background-color:#FF0000'";
                            } else if (rawObject[title.name] > vm.standEntity.perfStandWeek3){
                                return "style='background-color:#9BC2E6'";
                            }else if (rawObject[title.name] > vm.standEntity.perfStandWeek2){
                                return "style='background-color:#DDEBF7'";
                            }else if (rawObject[title.name] > vm.standEntity.perfStandWeek1 ){
                                return "style='background-color:#FCE4D6'";
                            }else if (0 <= rawObject[title.name] <= vm.standEntity.perfStandWeek1){
                                return "style='background-color:#F4B084'";
                            }
                        }else {
                            if (rawObject[title.name] === -1){
                                return "style='background-color:#FF0000'";
                            } else if (rawObject[title.name] > 630) {
                                return "style='background-color:#9BC2E6'";
                            } else if (rawObject[title.name] > 420) {
                                return "style='background-color:#DDEBF7'";
                            } else if (rawObject[title.name] > 210) {
                                return "style='background-color:#FCE4D6'";
                            } else if (0 <= rawObject[title.name] <= 210) {
                                return "style='background-color:#F4B084'";
                            }
                        }
                    }
            ...
            $.ajax({
                url: ctxPath + '/manager/F21029/getData',
                type: 'GET',
                dataType: "json",
                data: {
