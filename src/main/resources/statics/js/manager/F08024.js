...
var dist = {
    ...
    'tmplateTypeDiv': {
        label: 'テンプレートタイプ', name: 'tmplateTypeDiv', index: 'tmplateTypeDiv', width: 200, sortable: true, align: "center",
        formatter(cell, option, object) {
            var data = object.tmplateTypeDiv;
            return data;
        }
    },
    'tmplateTitle': {
...
// 表示項目全集合
var wholeList = ['tmplateCd','tmplateTypeDiv' ,'cretDatime', 'updDatime', 'ctgyNm', 'attachFilePath', 'cretUserName', 'updUserName', 'tmplateCnt', 'orgNm'];
...
var hiddenList = [];
// 条件を調べる
var tmplateId = null, tmplateTypeDiv=null,ctgyNm = null, cretDatimeStart = null, cretDatimeEnd = null,
    updDatimeStart = null, updDatimeEnd = null, cretUserName = null, updUserName = null;
// リスト重載
function reload(col, hidden, tmplateId_d,tmplateTypeDiv_d, ctgyNm_d, cretDatimeStart_d, cretDatimeEnd_d, updDatimeStart_d, updDatimeEnd_d, cretUserName_d, updUserName_d) {
    // 存储条件を調べる
    if (tmplateId_d != null)
        tmplateId = tmplateId_d;
    if (tmplateTypeDiv_d != null)
        tmplateTypeDiv = tmplateTypeDiv_d;
    if (ctgyNm_d != null)
        ctgyNm = ctgyNm_d;
    ...
    $("#jqGrid").jqGrid({
        url: ctxPath + '/manager/F08024/init',
        datatype: "json",
        postData: {
...
function deleteButton(id, updateStrForCheck) {
    var index = layer.confirm($.format($.msg.MSGCOMN0021, "削除"), {
        ...
        btn2: function () {
            $.post({
                url: ctxPath + '/manager/F08024/del',
                type: 'POST',
                data: {
                    id: id,
...
function editButton(id) {
    $(location).attr("href", "F08025.html?id=" + id);
}
...
function detailButton(id) {
    $.get(ctxPath + "/manager/F08024/getTmplateCont",
        {
            tmplateId: id
        }, function (data) {
            if (data.code == 0) {
...
$(function () {
    ...
    $("#search").click(function () {
        ...
        layer.open({
            ...
            area: [srcWidth, srcHeight],
            resize: false,
            content: ["../pop/F08024-1.html", 'no'],
            success: function (layero, index) {
                var body = layer.getChildFrame('body', index);
                var template = body.find('#template');
                var tmplateType = body.find('#tmplateType');
    ...
    $("#additem").click(function () {
        $(location).attr("href", "F08025.html");
    });
    // 表示項目初期化
    $.get(ctxPath + "/manager/F08024/getDspItems", function (rsp) {
        if (rsp.F08024DspDto) {
            wholeList = rsp.F08024DspDto.allItemslist;
            notDspList = rsp.F08024DspDto.mustItemslist;
            for (var i = 0; i < rsp.F08024DspDto.dspItemslist.length; i++) {
                colList.push(rsp.F08024DspDto.dspItemslist[i]);
            }
