...
$(function () {
    ...
    $("#jqGrid").jqGrid({
            url: ctxPath + '/manager/F04011/init',
            datatype: "json",
            postData: {},
            colModel: [
                {label: '組織ID', name: 'orgId', index: 'orgId', width: 40, key: true, sortable: false, align: "center"},
                {label: 'ＩＤ', name: 'id', index: 'id', width: 60, sortable: false, align: "center"},
                {
                    label: '重要度',
                    name: 'noticeLevelDiv',
                ...
                {label: '件名', name: 'noticeTitle', index: 'noticeTitle', width: 100, sortable: false, align: "center",
                    formatter(cell, option, object) {
                        return decodeURIComponent(object.noticeTitle);
                    }},
                {
                    label: '掲載期間',
                ...
                // },
                {
                    ...
                    formatter(cell, option, object) {
                        ...
                        if (object.orgFlg == 1) {
                            return "<button  onclick=window.location.href='./F04015.html?noticeId=" + object.id + "'> 確認 </button>&nbsp;&nbsp;"+"<button" +
                                "  onclick=window.location.href='./F04014.html?id=" + object.id + "'>詳細</button>&nbsp;&nbsp;" + "<button  onclick=window.location.href='./F04012.html?id=" + object.id + "'>編集</button>&nbsp;&nbsp;" + "<button  onclick='deleteButton(\"" + object.id + "\",\"" + object.updDatimeStr + "\"," + object.orgFlg + ");'>削除</button>";
                        } else {
                            return "<button  onclick=window.location.href='./F04014.html?id=" + object.id + "'>詳細</button>";
                        }
...
function searchButton() {
    times = 1;
    formatCheck();
    if ($("#f04011Form").valid() == true) {
        $("#jqGrid").jqGrid('setGridParam', {
            url: ctxPath + '/manager/F04011/search',
            datatype: "json",
            postData: {
                orgId: $('#orgId').val(),
                id: $('#id').val(),
...
function deleteButton(id, updDatimeStr) {
    var index = layer.confirm($.format($.msg.MSGCOMN0021, "削除"), {
        ...
        btn2: function () {
            $.ajax({
                url: ctxPath + '/manager/F04011/delete',
                type: 'POST',
                data: {
                    id: id,
