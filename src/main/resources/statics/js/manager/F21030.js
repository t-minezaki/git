    ...
    methods: {
        setUp: function () {
            ...
            $('#nextWeek').click(function () {
                vm.isFirst = true;
                vm.pageTurning = true;
                vm.date.setMonth(vm.date.getMonth() + 7);
                vm.getResult();
            });
            //「先月」アイコンのイベントをクリック
        ...
        getResult: function () {
            ...
            for (var i = 0; i < 7; i++) {
                ...
                columns.push({
                    ...
                    cellattr: function (rowId, val, rawObject, title, rdata) {
                        ...
                        if (vm.standEntity.perfStandMonth3 && vm.standEntity.perfStandMonth2 && vm.standEntity.perfStandMonth1) {
                            if (rawObject[title.name] === -1){
                                return "style='background-color:#FF0000'";
                            } else if (rawObject[title.name] > vm.standEntity.perfStandMonth3) {
                                return "style='background-color:#9BC2E6'";
                            } else if (rawObject[title.name] > vm.standEntity.perfStandMonth2) {
                                return "style='background-color:#DDEBF7'";
                            } else if (rawObject[title.name] > vm.standEntity.perfStandMonth1 ) {
                                return "style='background-color:#FCE4D6'";
                            } else if (0 <= rawObject[title.name] <= vm.standEntity.perfStandMonth1) {
                                return "style='background-color:#F4B084'";
                            }
                        }else {
                            if (rawObject[title.name] === -1){
                                return "style='background-color:#FF0000'";
                            } else if (rawObject[title.name] > 2700) {
                                return "style='background-color:#9BC2E6'";
                            } else if (rawObject[title.name] > 1800) {
                                return "style='background-color:#DDEBF7'";
                            } else if (rawObject[title.name] > 900 ) {
                                return "style='background-color:#FCE4D6'";
                            } else if (0 <= rawObject[title.name] <= 900) {
                                return "style='background-color:#F4B084'";
                            }
                        }
                    }
            ...
            $.ajax({
                url: ctxPath + '/manager/F21030/getData',
                type: 'GET',
                dataType: "json",
                data: {
