...
$(function () {
    ...
    $("#jqGrid").jqGrid({
            url: ctxPath + '/manager/F21062/init',
            datatype: "json",
            postData: {},
            colModel: [
                {label: '組織ID', name: 'orgId', index: 'orgId', width: 40, key: true, sortable: false, align: "center"},
                {label: 'メッセージＩＤ', name: 'id', index: 'id', width: 50, sortable: false, align: "center"},
                {
                    label: '重要度', name: 'messageLevelDiv', index: 'messageLevelDiv', width: 30, sortable: false, align: "center",
                ...
                },
                {
                    ...
                    formatter(cell, option, object) {
                        ...
                        if (object.orgFlg == 1) {
                            return "<button onclick=window.location.href='./F21065.html?id=" + object.id + "'> 確認 </button>&nbsp;&nbsp;"+"<button" +
                                "  onclick=window.location.href='./F21067.html?id=" + object.id + "'>詳細</button>&nbsp;&nbsp;" + "<button  onclick=window.location.href='./F21064.html?id=" + object.id + "'>編集</button>&nbsp;&nbsp;" + "<button  onclick='deleteButton(\"" + object.id + "\",\"" + object.updDatimeStr + "\"," + object.orgFlg + ");'>削除</button>";
                        } else {
                            return "<button  onclick=window.location.href='./F21067.html?id=" + object.id + "'>詳細</button>";
                        }
...
function searchButton() {
    ...
    if ($("#f21062Form").valid() == true) {
        $("#jqGrid").jqGrid('setGridParam', {
            url: ctxPath + '/manager/F21062/search',
            datatype: "json",
            postData: {
                orgId: $('#orgId').val(),
                id: $('#id').val(),
...
function deleteButton(id, updDatimeStr) {
    var index = layer.confirm($.format($.msg.MSGCOMN0021, "削除"), {
        ...
        btn2: function () {
            $.ajax({
                url: ctxPath + '/manager/F21062/delete',
                type: 'POST',
                data: {
                    id: id,
...
function formatCheck() {
    $("#f21062Form").validate({
        ...
        messages: {
            ...
            // メッセージID
            id: {
                number: $.format($.msg.MSGCOMD0006, "メッセージID")
            }
        },
