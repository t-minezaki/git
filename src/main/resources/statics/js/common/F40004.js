    ...
    mounted: function(){
        $.ajax({
            ...
            success: function (data){
                vue.orgAddId = data.orgIdAdd.toUpperCase();
            }
        });
        this.pd();
    },
            ...
            $.ajax({
                ...
                success:function (data) {
                    if (data.code == 0){
                        ...
                        var brand = getCookie("brandcd").toUpperCase()==='GKGC'?getCookie("brandcd").toUpperCase():getCookie("brandcd");
                        var dataBrand = data.brandCd.toUpperCase() ==='GKGC'?data.brandCd.toUpperCase():data.brandCd;
                        vue.brandCd = data.brandCd == ''?brand:dataBrand;
                    }else if (data.code == 307) {
                        parent.window.location.href = ctxPath + data.msg;
                    }
                    $("#v_If").css("display","block");
                }
            });
        },
...
function isActive() {
    ...
    if(param['id']=='F30411' || param.id === 'F30412'){
        for(var i=0;i< $("li").length;i++){
            if(i!=2){
                $("li").eq(i).find("img").eq(0).attr("src","../img/bottomPngs8.0/bottompng8.0_"+(i+1)+".png");
            }else{
    ...
    if(param['id']=='F30406' || param['id']=='F30422'){
        for(var i=0;i< $("li").length;i++){
            if(i!=3){
                $("li").eq(i).find("img").eq(0).attr("src","../img/bottomPngs8.0/bottompng8.0_"+(i+1)+".png");
            }else{
