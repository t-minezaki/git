...
@RequestMapping("/manager/F05001")
@RestController
public class F05001Controller {

    /**
     * 知らせ一覧画面
     */
    @Autowired
    private F05001Service f05001Service;
    ...
    public R f05001Delete(Integer id, String updDatimeStr) {
        //お知らせ．IDの取得
        ...
        if (mstNoticeEntity != null && DateUtils.format(mstNoticeEntity.getUpdDatime(), GakkenConstant.DATE_FORMAT_YYYYMMDDHHMMSSSSS).equals(updDatimeStr)) {
            ...
            threadPoolTaskExecutor.execute(() -> {
                ...
                mstNoticeDeliverEntity.setUpdUsrId(ShiroUtils.getUserEntity().getOrgId());
                ...
                guardReadingStsEntity.setUpdUsrId(ShiroUtils.getUserEntity().getOrgId());
     ...
     * @param page 現在のページ数
     *
     * update 2020/12/1  NWT文　ロジックを最適化
     * @return
     */
    private R getDetail(String sOrgId, String orgId, Integer id, String noticeTitle, Date pubPlanStartDt, Date pubPlanEndDt, Integer limit, Integer page) {
        //総件数をとる
        Integer total = f05001Service.getTotalCount(sOrgId, orgId, id, noticeTitle, pubPlanStartDt, pubPlanEndDt);
        if (total == 0) {
            return R.error(MessageUtils.getMessage("MSGCOMN0017", "お知らせ"));
        }
        List<F05001Dto> list = f05001Service.getSearchOrg(sOrgId, orgId, id, noticeTitle, pubPlanStartDt, pubPlanEndDt, limit, (page - 1) * limit);
        for (F05001Dto dto : list) {
