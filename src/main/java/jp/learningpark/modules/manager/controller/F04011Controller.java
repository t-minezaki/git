 ...
 * @version 6.0
 */
@RequestMapping("/manager/F04011")
@RestController
public class F04011Controller {
    /**
     * 塾ニュース一覧画面
     */
    @Autowired
    private F04011Service f04011Service;
    ...
    private R getDetail(String sOrgId, String orgId, Integer id, String noticeTitle, Date pubPlanStartDt, Date pubPlanEndDt, Integer limit, Integer page) {
        //総件数をとる
        Integer total = f04011Service.getTotalCount(sOrgId, orgId, id, noticeTitle, pubPlanStartDt, pubPlanEndDt);
        if (total == 0) {
            return R.error(MessageUtils.getMessage("MSGCOMN0017", "マナミルチャンネル"));
        }
        List<F04011Dto> list = f04011Service.getSearchOrg(sOrgId, orgId, id, noticeTitle, pubPlanStartDt, pubPlanEndDt, limit, (page - 1) * limit);
        for (F04011Dto dto : list) {
