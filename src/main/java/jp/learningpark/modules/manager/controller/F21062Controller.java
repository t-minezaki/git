 ...
 */
@RequestMapping("/manager/F21062")
@RestController
public class F21062Controller {

    @Autowired
    F21062Service f21062Service;
    /**
     * 組織マスタ Service
     */
    @Autowired
    ...
    public R init(Integer limit, Integer page) {
        ...
        R r = f21062Service.getDetail(ShiroUtils.getUserEntity().getOrgId(), null, null, null, null, null, limit, page);
    ...
    public R search(String orgId, Integer id, String messageTitle, String pubPlanStartDt, String pubPlanEndDt, Integer limit, Integer page) {
        if (pubPlanStartDt != null && !StringUtils.isEmpty(pubPlanStartDt) && pubPlanEndDt != null && !StringUtils.isEmpty(pubPlanEndDt)) {
            ...
            return f21062Service.getDetail(ShiroUtils.getUserEntity().getOrgId(), orgId, id, messageTitle, DateUtils.parse(pubPlanStartDt, GakkenConstant.DATE_FORMAT_YYYY_MM_DD_HH_MM), DateUtils.parse(pubPlanEndDt, GakkenConstant.DATE_FORMAT_YYYY_MM_DD_HH_MM), limit, page);
        } else {
            if (StringUtils.isEmpty(pubPlanStartDt)) {
                if (!StringUtils.isEmpty(pubPlanEndDt)) {
                    return f21062Service.getDetail(ShiroUtils.getUserEntity().getOrgId(), orgId, id, messageTitle, null, DateUtils.parse(pubPlanEndDt, GakkenConstant.DATE_FORMAT_YYYY_MM_DD_HH_MM), limit, page);
                } else {
                    return f21062Service.getDetail(ShiroUtils.getUserEntity().getOrgId(), orgId, id, messageTitle, null, null, limit, page);
                }
            } else {
                if (StringUtils.isEmpty(pubPlanEndDt)) {
                    return f21062Service.getDetail(ShiroUtils.getUserEntity().getOrgId(), orgId, id, messageTitle, DateUtils.parse(pubPlanStartDt, GakkenConstant.DATE_FORMAT_YYYY_MM_DD_HH_MM), null, limit, page);
                }
            }
        }
        return R.ok();
