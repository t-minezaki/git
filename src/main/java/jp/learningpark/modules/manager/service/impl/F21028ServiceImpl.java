...
import jp.learningpark.modules.manager.service.F21028Service;
 ...
 * @date 2020/02/13 12:05
 */
@Service
public class F21028ServiceImpl implements F21028Service {
    ...
    public R getLineDate(Map<String, Object> params) {
        ...
        if ((Boolean) params.get("isFirst")){
            ...
            if (!(Boolean) params.get("pageTurning")) {
                ...
                PerfStandMstEntity perfStandMstEntity = perfStandMstDao.selectOne(new QueryWrapper<PerfStandMstEntity>().select("perf_stand_day1", "perf_stand_day2", "perf_stand_day3").eq("org_id", params.get("orgId")).eq("usr_id", params.get("usrId")).eq("del_flg", 0));
            ...
            if ((Boolean) params.get("flag") && !(Boolean) params.get("pageTurning") && lineData.size() == 0){
        ...
        if (lineData.size() > 0){
            ...
            //学習時間データを取得する
            List<F21028Dto> dataByDay = f21028Dao.getDataByDay(stuIdList, startDate, endDate);
//            if (!(Boolean) params.get("isFirst")) {
//                //初期化されていない && 学習データは0です
//                if (dataByDay.size() == 0) {
...
//            }
            ...
            for (int i = 0; i < 7; i++){
                dayIndexMap.put(DateUtils.addDays(startDate, i), 6 - i);
            }
            //データ入力
            dataByDay.forEach(v -> {
