 ...
 */
@Service
public class F21030ServiceImpl implements F21030Service {
    ...
    public R getLineDate(Map<String, Object> params) {
        ...
        if ((Boolean) params.get("isFirst")){
            ...
            if (!(Boolean) params.get("pageTurning")) {
                ...
                PerfStandMstEntity perfStandMstEntity = perfStandMstDao.selectOne(new QueryWrapper<PerfStandMstEntity>().select("perf_stand_month1", "perf_stand_month2", "perf_stand_month3").eq("org_id", params.get("orgId")).eq("usr_id", params.get("usrId")).eq("del_flg", 0));
        ...
        if (lineData.size() > 0){
            ...
            //maxDate
            Date endDate = DateUtils.addDays(DateUtils.addMonths(startDate, 7), -1);
            //学習時間データを取得する
            List<F21028Dto> dataByDay = f21028Dao.getDataByMonth(stuIdList, startDate, endDate);
//            if (!(Boolean) params.get("isFirst")) {
//                //初期化されていない && 学習データは0です
//                if (dataByDay.size() == 0) {
...
//            }
            ...
            for (int i = 0; i < 7; i++){
                dayIndexMap.put(DateUtils.addMonths(startDate, i), 6 - i);
            }
            //データ入力
            dataByDay.forEach(v -> {
